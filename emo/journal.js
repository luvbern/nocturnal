document.addEventListener("DOMContentLoaded", () => {
    ...
    function paginateJournal() {
        ...
        filteredEntries.forEach(entry => {
            ...
            const childElements = entry.querySelectorAll("p, div:not(.polaroid-container):not(.polaroid):not(.polaroid-img-placeholder):not(.tape)");
            const polaroid = entry.querySelector(".polaroid-container");
            
            let currentPageHTML = timeTag;

            if (childElements.length === 0 && entry.innerText.trim() !== "") {
                let txt = entry.innerHTML.replace(timeTag, "");
                currentPageHTML += txt;
            ...
            childElements.forEach(el => {
                const sentences = el.innerHTML.match(/(<img[^>]*>|<[^>]+>|[^.!?<]+(?:[.!?]+(?=\s|<br\/?>|$))?|[^.!?<]+)/g) || [el.innerHTML];
                let paragraphBuffer = "";
            ...
            if (polaroid) {
                let testWithPolaroid = currentPageHTML + polaroid.outerHTML;
                ...
                if (tester.scrollHeight > maxHeight) {
                    ...
                    currentPageHTML = polaroid.outerHTML;
                } else {
                    currentPageHTML += polaroid.outerHTML;
                }
            }
